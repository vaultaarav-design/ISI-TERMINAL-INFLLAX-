...
// Tracks which Bias-Engine / SMC fields were auto-picked by Section 1's zone
// math (vs. clicked manually by the trader). An auto-picked field keeps
// refreshing as the zone inputs change; the moment the trader clicks a
// button themselves, its flag flips to false and Smart Logic stops touching it.
const peAutoFlags = { htf_ms: false, htf_zone: false, ltf_ms: false, ltf_candle: false, mstate: false, vol: false, smm: {} };
...
window.updateEntryZoneValidation = function () {
    ...
    const meterWrapEl = document.getElementById('ezvReadinessWrap');
    if (!box) return;
    ...
    if (!entry || isNaN(htfHighR) || isNaN(htfLowR) || isNaN(ltfHighR) || isNaN(ltfLowR)) {
        ...
        if (meterWrapEl) meterWrapEl.style.display = 'none';
        hidePeAutoNotes();
        return;
    }
    ...
    // Zone position % — 0% = at the low/support edge, 100% = at the high/resistance edge.
    // Raw keeps values <0 / >100 when price is outside the zone (used for bar placement);
    // clamped is used for the tag maths below.
    const htfPctRaw = htfHi > htfLo ? Math.round((entry - htfLo) / (htfHi - htfLo) * 100) : 50;
    const ltfPctRaw = ltfHi > ltfLo ? Math.round((entry - ltfLo) / (ltfHi - ltfLo) * 100) : 50;
    const htfPct = clampPct(htfPctRaw);
    const ltfPct = clampPct(ltfPctRaw);

    // Same number, read both ways — buy-side % from support, sell-side % from resistance —
    // so it reads correctly whichever direction the trader is thinking in.
    const htfBuyPct  = htfPct,       htfSellPct = 100 - htfPct;
    const ltfBuyPct  = ltfPct,       ltfSellPct = 100 - ltfPct;

    // Freshness — is this a fresh reaction at the zone edge (institutional-style)
    ...
    // ── HTF BREAKOUT / STRUCTURE STATUS ──
    // Is price trading ABOVE the HTF zone (resistance broken → BOS bullish),
    // BELOW it (support broken → BOS bearish), or still INSIDE it (range, or
    // a CHoCH possibly forming right at the edge)?
    let htfBreakout, htfStructTag, htfStructColor, htfStructText;
    if (entry > htfHi) {
        htfBreakout = 'ABOVE'; htfStructTag = 'BOS_BULL'; htfStructColor = 'var(--accent)';
        htfStructText = `🟢 Price trading ABOVE the HTF (${htfTf||'—'}) zone — resistance broken → <b>BOS Bullish</b> (structure flipped: old resistance now acts as support).`;
    } else if (entry < htfLo) {
        htfBreakout = 'BELOW'; htfStructTag = 'BOS_BEAR'; htfStructColor = 'var(--danger)';
        htfStructText = `🔴 Price trading BELOW the HTF (${htfTf||'—'}) zone — support broken → <b>BOS Bearish</b> (structure flipped: old support now acts as resistance).`;
    } else {
        htfBreakout = 'INSIDE';
        const impliedDir = direction || (htfPct <= 50 ? 'LONG' : 'SHORT');
        if (impliedDir === 'LONG' && htfPct <= 35) {
            htfStructTag = 'CHoCH_BULL'; htfStructColor = 'var(--gold)';
            htfStructText = `🟡 Price INSIDE the HTF zone, near the low — possible <b>CHoCH</b> forming (bullish reversal off support).`;
        } else if (impliedDir === 'SHORT' && htfPct >= 65) {
            htfStructTag = 'CHoCH_BEAR'; htfStructColor = 'var(--gold)';
            htfStructText = `🟡 Price INSIDE the HTF zone, near the high — possible <b>CHoCH</b> forming (bearish reversal off resistance).`;
        } else {
            htfStructTag = 'RANGE'; htfStructColor = '#888';
            htfStructText = `⚪ Price INSIDE the HTF zone, mid-range — ranging between support &amp; resistance, no break yet.`;
        }
    }

    // HTF price-zone label (Demand/Discount/EQ/Premium/Supply) from position %
    let htfZoneTag;
    if (htfPct <= 20) htfZoneTag = 'DEMAND';
    else if (htfPct <= 45) htfZoneTag = 'DISCOUNT';
    else if (htfPct < 55) htfZoneTag = 'EQ';
    else if (htfPct < 80) htfZoneTag = 'PREMIUM';
    else htfZoneTag = 'SUPPLY';

    // ── LTF's OWN breakout status (independent read: entry vs the LTF zone
    //    itself, ignoring the HTF zone) — this is what lets us later say
    //    "auto-picked because of HTF" vs "because of LTF" vs "both agree".
    let ltfOwnBreakout; // 'ABOVE_LTF' | 'BELOW_LTF' | 'INSIDE_LTF'
    if (entry > ltfHi) ltfOwnBreakout = 'ABOVE_LTF';
    else if (entry < ltfLo) ltfOwnBreakout = 'BELOW_LTF';
    else ltfOwnBreakout = 'INSIDE_LTF';

    // ── LTF POSITION vs HTF BREAKOUT — green zone / red zone / mid zone ──
    let ltfZoneStatus, ltfZoneColor, ltfZoneText;
    if (entry > htfHi) {
        ltfZoneStatus = 'GREEN'; ltfZoneColor = 'var(--accent)';
        ltfZoneText = `🟢 GREEN ZONE — LTF price is trading above the HTF breakout level.`;
    } else if (entry < htfLo) {
        ltfZoneStatus = 'RED'; ltfZoneColor = 'var(--danger)';
        ltfZoneText = `🔴 RED ZONE — LTF price is trading below the HTF breakdown level.`;
    } else if (ltfOwnBreakout === 'ABOVE_LTF') {
        ltfZoneStatus = 'MID'; ltfZoneColor = 'var(--gold)';
        ltfZoneText = `🟡 MID ZONE — inside HTF range, above the LTF zone (near the LTF resistance side).`;
    } else if (ltfOwnBreakout === 'BELOW_LTF') {
        ltfZoneStatus = 'MID'; ltfZoneColor = 'var(--gold)';
        ltfZoneText = `🟡 MID ZONE — inside HTF range, below the LTF zone (near the LTF support side).`;
    } else {
        ltfZoneStatus = 'MID'; ltfZoneColor = 'var(--gold)';
        ltfZoneText = `🟡 MID ZONE — inside both the HTF range and the LTF zone (confirmation area).`;
    }

    // LTF structure/candle tags — feed Section 2's LTF pane
    let ltfStructTag, ltfCandleTag;
    if (ltfZoneStatus === 'GREEN' || ltfZoneStatus === 'RED') {
        ltfStructTag = 'EXPANSION'; ltfCandleTag = 'IMPULSE';
    } else if (ltfOwnBreakout === 'INSIDE_LTF') {
        const impliedDir = direction || (ltfPct <= 50 ? 'LONG' : 'SHORT');
        if (impliedDir === 'LONG' && ltfPct <= 35)       { ltfStructTag = 'CHoCH_BULL'; ltfCandleTag = 'REJECTION'; }
        else if (impliedDir === 'SHORT' && ltfPct >= 65) { ltfStructTag = 'CHoCH_BEAR'; ltfCandleTag = 'REJECTION'; }
        else                                              { ltfStructTag = 'CONTRACTION'; ltfCandleTag = 'NO_SIGNAL'; }
    } else {
        ltfStructTag = 'CONTRACTION'; ltfCandleTag = 'PINBAR';
    }


    // ── ENTRY READINESS METER (0-100) ──
    // Distinct from the big Institutional Readiness Score in Section 6 — this
    // one is purely about *where inside the zones* this specific entry sits.
    let meter = 50;
    if (inHtf && inLtf) meter += 20; else if (inHtf || inLtf) meter += 5; else meter -= 25;
    if (freshness?.startsWith('FRESH')) meter += 20;
    else if (freshness?.startsWith('LATE')) meter -= 20;
    if (direction === 'LONG') {
        if (htfBreakout === 'BELOW') meter -= 30;       // buying against a broken support
        else if (htfBreakout === 'ABOVE') meter -= 10;  // chasing an already-broken high
    } else if (direction === 'SHORT') {
        if (htfBreakout === 'ABOVE') meter -= 30;
        else if (htfBreakout === 'BELOW') meter -= 10;
    }
    meter = Math.max(0, Math.min(100, Math.round(meter)));

    let meterLabel, meterColor;
    if (meter >= 80)      { meterLabel = '💎 EXCELLENT — Textbook Institutional Entry'; meterColor = 'var(--accent)'; }
    else if (meter >= 60) { meterLabel = '🟢 GOOD — Institutional-Style Entry';         meterColor = 'var(--accent)'; }
    else if (meter >= 35) { meterLabel = '🟡 MODERATE — Proceed With Caution';          meterColor = 'var(--gold)';   }
    else                   { meterLabel = '🔴 LOW — Retail Chase Risk';                  meterColor = 'var(--danger)'; }

    if (meterWrapEl) {
        meterWrapEl.style.display = 'block';
        const lbl = document.getElementById('ezvReadinessLabel');
        lbl.textContent = `${meter}/100 — ${meterLabel}`;
        lbl.style.color = meterColor;
        const fillEl = document.getElementById('ezvReadinessFill');
        fillEl.style.width = meter + '%';
        fillEl.style.background = meterColor;
    }
    ...
    box.innerHTML = `
        ...
        <div class="ezv-bar-wrap"><div class="ezv-bar-fill" style="left:${Math.max(0,Math.min(100,htfPctRaw))}%;background:${color};"></div></div>
        <div style="display:flex;justify-content:space-between;font-size:0.52rem;color:#555;margin-top:3px;">
            <span>HTF Support</span><span>Buy Zone: ${htfBuyPct}% · Sell Zone: ${htfSellPct}%</span><span>HTF Resistance</span>
        </div>

        <div class="ezv-ltf-bar-wrap"><div class="ezv-ltf-bar-fill" style="left:${Math.max(0,Math.min(100,ltfPctRaw))}%;"></div></div>
        <div style="display:flex;justify-content:space-between;font-size:0.52rem;color:#555;margin-top:3px;">
            <span>LTF Support</span><span>Buy Zone: ${ltfBuyPct}% · Sell Zone: ${ltfSellPct}%</span><span>LTF Resistance</span>
        </div>

        <div style="margin-top:10px;padding-top:10px;border-top:1px dashed #222;color:${htfStructColor};font-size:0.68rem;line-height:1.5;">${htfStructText}</div>
        <div style="margin-top:6px;color:${ltfZoneColor};font-size:0.68rem;line-height:1.5;">${ltfZoneText}</div>
    `;
    ...
    peData.entryZoneValidation = {
        ...
        htfBuyPct, htfSellPct, ltfBuyPct, ltfSellPct,
        tag: tag.replace(/^[^ ]+\s/, ''), freshness,
        htfBreakout, htfStructTag, htfZoneTag,
        ltfZoneStatus, ltfStructTag, ltfCandleTag,
        readinessMeter: meter, readinessLabel: meterLabel,
    };

    // Feed everything computed above into Section 2 (Bias Engine), Section 3
    // (Smart Money Concepts) and Section 4 (Market State & Volatility) —
    // auto-selecting buttons unless the trader has already picked something
    // manually for that slot, and always labelling *why* (HTF / LTF / both).
    runSmartZoneAutoLogic({
        htfStructTag, htfZoneTag, ltfStructTag, ltfCandleTag, ltfZoneStatus,
        htfBreakout, ltfOwnBreakout, htfPct, ltfPct, direction,
    });
};

function hidePeAutoNotes() {
    const n1 = document.getElementById('htfLtfAutoNote'); if (n1) n1.style.display = 'none';
    const n2 = document.getElementById('smcAutoNote');    if (n2) n2.style.display = 'none';
    const n3 = document.getElementById('mstateAutoNote'); if (n3) n3.style.display = 'none';
}

// Selects a struct-btn programmatically (same visual result as a manual click)
// and tags it as auto-picked, so it can be refreshed later or overridden.
function applyStructAuto(tf, key, val) {
    const btn = document.querySelector(`.struct-btn[data-tf="${tf}"][data-key="${key}"][data-val="${val}"]`);
    if (!btn) return;
    const typ = btn.dataset.type;
    document.querySelectorAll(`.struct-btn[data-tf="${tf}"][data-key="${key}"]`).forEach(b => {
        b.classList.remove('active-bull', 'active-bear', 'active-neut', 'auto-picked');
    });
    btn.classList.add(`active-${typ}`, 'auto-picked');
    if (!peData[tf]) peData[tf] = {};
    peData[tf][key] = val;
    peAutoFlags[`${tf}_${key}`] = true;
}

// Selects a Market State button programmatically.
function applyMstateAuto(val) {
    const btn = document.querySelector(`.mstate-btn[data-val="${val}"]`);
    if (!btn) return;
    document.querySelectorAll('.mstate-btn').forEach(b => b.classList.remove('sel-bull', 'sel-bear', 'sel-neut', 'auto-picked'));
    btn.classList.add(btn.dataset.cls, 'auto-picked');
    peData.mstate = val;
    peAutoFlags.mstate = true;
}

// Selects a Volatility button programmatically.
function applyVolAuto(val) {
    const btn = document.querySelector(`.struct-btn[data-key="vol"][data-val="${val}"]`);
    if (!btn) return;
    document.querySelectorAll('[data-key="vol"]').forEach(b => b.classList.remove('active-bull', 'active-bear', 'active-neut', 'auto-picked'));
    btn.classList.add('active-neut', 'auto-picked');
    peData.volatility = val;
    peAutoFlags.vol = true;
}

// Toggles a Smart Money Concept ON and stamps a small "HTF / LTF / HTF+LTF"
// reason tag directly onto the button, so it's visible *why* it was picked
// without having to open a separate log.
function applySmmAuto(key, reason) {
    const btn = document.querySelector(`.smm-btn[data-key="${key}"]`);
    if (!btn) return;
    if (!btn.classList.contains('sel')) {
        btn.classList.add('sel', 'auto-picked');
        peData.smm[key] = true;
        peAutoFlags.smm[key] = true;
    }
    setSmmReasonTag(btn, reason);
}
function setSmmReasonTag(btn, reason) {
    let tagEl = btn.querySelector('.smm-reason');
    if (!tagEl) {
        tagEl = document.createElement('div');
        tagEl.className = 'smm-reason';
        btn.appendChild(tagEl);
    }
    const color = reason === 'HTF+LTF' ? 'var(--accent)' : reason === 'HTF' ? '#4a9eff' : 'var(--gold)';
    const label = reason === 'HTF+LTF' ? 'HTF + LTF confluence' : reason === 'HTF' ? 'HTF-driven' : 'LTF-driven';
    tagEl.textContent = `🤖 ${label}`;
    tagEl.style.color = color;
}
function clearSmmReasonTag(btn) {
    const tagEl = btn.querySelector('.smm-reason');
    if (tagEl) tagEl.remove();
}

// ── PER-TIMEFRAME SMC READ ──
// Reads one timeframe's zone in isolation (own breakout status + own zone
// position %) and returns which Smart Money Concepts that timeframe alone
// would justify. Called once for HTF and once for LTF so we can tell exactly
// which timeframe is responsible for each auto-pick (or if both agree).
function computeTfSmcSignal(breakoutStatus, pct, direction) {
    const impliedDir = direction || (pct <= 50 ? 'LONG' : 'SHORT');
    if (breakoutStatus === 'ABOVE') {
        return { keys: ['liqHunt', 'inducement'] };
    }
    if (breakoutStatus === 'BELOW') {
        return { keys: ['liqHunt', 'inducement'] };
    }
    // INSIDE the zone — is this a fresh edge reaction or dead mid-zone air?
    const freshEdge = impliedDir === 'LONG' ? pct <= 30 : pct >= 70;
    if (freshEdge) {
        return impliedDir === 'LONG' ? { keys: ['orderBlock', 'accumulation'] } : { keys: ['orderBlock', 'distribution'] };
    }
    return { keys: ['fvg'] };
}

// Applies Smart Logic's auto-picks to Section 2 (Bias Engine), Section 3
// (Smart Money Concepts) and Section 4 (Market State & Volatility) — but only
// for slots that are still empty or were themselves last set by Smart Logic.
// A manual click always wins and is never overwritten by a later recalc.
function runSmartZoneAutoLogic({ htfStructTag, htfZoneTag, ltfStructTag, ltfCandleTag, ltfZoneStatus, htfBreakout, ltfOwnBreakout, htfPct, ltfPct, direction }) {
    let structTouched = false;

    // ── Section 2 — HTF/LTF Bias Engine ──
    if (!peData.htf.ms || peAutoFlags.htf_ms)         { applyStructAuto('htf', 'ms', htfStructTag);     structTouched = true; }
    if (!peData.htf.zone || peAutoFlags.htf_zone)     { applyStructAuto('htf', 'zone', htfZoneTag);     structTouched = true; }
    if (!peData.ltf.ms || peAutoFlags.ltf_ms)         { applyStructAuto('ltf', 'ms', ltfStructTag);     structTouched = true; }
    if (!peData.ltf.candle || peAutoFlags.ltf_candle) { applyStructAuto('ltf', 'candle', ltfCandleTag); structTouched = true; }

    const impliedDir = direction || (htfPct <= 50 ? 'LONG' : 'SHORT');

    // ── Section 3 — Smart Money Concepts ──
    // Big funds don't read one chart in isolation — they read HTF and LTF
    // price action *against each other*. So: work out what HTF's own zone
    // says, what LTF's own zone says, and merge — a key that both timeframes
    // agree on is a genuine confluence read (HTF+LTF); a key only one
    // timeframe supports is a weaker, single-timeframe read (HTF or LTF).
    const htfSig = computeTfSmcSignal(htfBreakout, htfPct, direction);
    const ltfOwnStatus = ltfOwnBreakout === 'ABOVE_LTF' ? 'ABOVE' : ltfOwnBreakout === 'BELOW_LTF' ? 'BELOW' : 'INSIDE';
    const ltfSig = computeTfSmcSignal(ltfOwnStatus, ltfPct, direction);

    const smcReasons = {}; // key -> 'HTF' | 'LTF' | 'HTF+LTF'
    htfSig.keys.forEach(k => { smcReasons[k] = 'HTF'; });
    ltfSig.keys.forEach(k => { smcReasons[k] = smcReasons[k] ? 'HTF+LTF' : 'LTF'; });

    // Wyckoff Spring / UTAD — HTF zone still holds (no real break) while LTF
    // wicks beyond its OWN extreme first. Classic large-player pattern: grab
    // the LTF stops, then reverse inside the still-intact HTF zone.
    if (htfBreakout === 'INSIDE') {
        if (impliedDir === 'LONG' && htfPct <= 40 && ltfOwnBreakout === 'BELOW_LTF') {
            smcReasons.wyckoffSpring = 'HTF+LTF'; smcReasons.stopHunt = 'HTF+LTF';
        } else if (impliedDir === 'SHORT' && htfPct >= 60 && ltfOwnBreakout === 'ABOVE_LTF') {
            smcReasons.wyckoffSpring = 'HTF+LTF'; smcReasons.stopHunt = 'HTF+LTF';
        }
    }
    // HTF and LTF actively disagreeing (one broke bullish, the other bearish,
    // in the same read) — flag as manipulation rather than a clean setup.
    if ((htfBreakout === 'ABOVE' && ltfOwnBreakout === 'BELOW_LTF') ||
        (htfBreakout === 'BELOW' && ltfOwnBreakout === 'ABOVE_LTF')) {
        smcReasons.manipulation = 'HTF+LTF';
    }

    let smcTouched = false;
    Object.entries(smcReasons).forEach(([key, reason]) => {
        if (!peData.smm[key] || peAutoFlags.smm[key]) {
            const wasOn = !!peData.smm[key];
            applySmmAuto(key, reason);
            if (!wasOn) smcTouched = true;
        }
    });

    // ── Section 4 — Market State & Volatility ──
    // Same HTF-vs-LTF comparison, now read as a market regime instead of a
    // footprint tag: trending, ranging, compressing before a break, or set
    // up for a reversal.
    let mstateVal, mstateReason, volVal, volReason;
    if (htfBreakout === 'ABOVE') {
        if (ltfOwnBreakout === 'ABOVE_LTF') { mstateVal = 'TREND_BULL';    mstateReason = 'HTF+LTF'; volVal = 'HIGH'; volReason = 'HTF+LTF'; }
        else                                 { mstateVal = 'POST_BREAKOUT'; mstateReason = 'HTF';      volVal = 'HIGH'; volReason = 'HTF'; }
    } else if (htfBreakout === 'BELOW') {
        if (ltfOwnBreakout === 'BELOW_LTF') { mstateVal = 'TREND_BEAR';    mstateReason = 'HTF+LTF'; volVal = 'HIGH'; volReason = 'HTF+LTF'; }
        else                                 { mstateVal = 'POST_BREAKOUT'; mstateReason = 'HTF';      volVal = 'HIGH'; volReason = 'HTF'; }
    } else if (ltfOwnBreakout !== 'INSIDE_LTF') {
        // HTF zone holding but LTF already pushing past its own box —
        // compression inside a bigger range, ready to break either way.
        mstateVal = 'PRE_BREAKOUT'; mstateReason = 'HTF+LTF'; volVal = 'LOW'; volReason = 'HTF+LTF';
    } else {
        const htfFreshEdge = impliedDir === 'LONG' ? htfPct <= 30 : htfPct >= 70;
        const ltfFreshEdge = impliedDir === 'LONG' ? ltfPct <= 30 : ltfPct >= 70;
        if (htfFreshEdge && ltfFreshEdge) { mstateVal = 'REVERSAL_SETUP'; mstateReason = 'HTF+LTF'; volVal = 'HIGH';   volReason = 'HTF+LTF'; }
        else if (htfFreshEdge)            { mstateVal = 'REVERSAL_SETUP'; mstateReason = 'HTF';      volVal = 'NORMAL'; volReason = 'HTF'; }
        else                                { mstateVal = 'RANGE';          mstateReason = 'HTF+LTF'; volVal = 'NORMAL'; volReason = 'HTF+LTF'; }
    }

    let mstateTouched = false;
    if (!peData.mstate || peAutoFlags.mstate) { applyMstateAuto(mstateVal); mstateTouched = true; }
    if (!peData.volatility || peAutoFlags.vol) { applyVolAuto(volVal); mstateTouched = true; }

    if (mstateTouched) {
        const n = document.getElementById('mstateAutoNote');
        if (n) {
            n.style.display = 'block';
            n.innerHTML = `🤖 Market State auto-picked <b style="color:${mstateReason==='HTF+LTF'?'var(--accent)':'#4a9eff'}">(${mstateReason==='HTF+LTF'?'HTF + LTF confluence':'HTF-driven'})</b> ·
                Volatility auto-picked <b style="color:${volReason==='HTF+LTF'?'var(--accent)':'#4a9eff'}">(${volReason==='HTF+LTF'?'HTF + LTF confluence':'HTF-driven'})</b> — click any button to override.`;
        }
    }

    if (structTouched) { const n = document.getElementById('htfLtfAutoNote'); if (n) n.style.display = 'block'; }
    if (smcTouched)    { const n = document.getElementById('smcAutoNote');    if (n) n.style.display = 'block'; }

    if (structTouched || smcTouched || mstateTouched) {
        checkConflict();
        updateBiasResult();
        recalcScore();
    }
...
window.setStruct = function (btn) {
    ...
    if (!peData[tf]) peData[tf] = {};
    peData[tf][key] = val;
    peAutoFlags[`${tf}_${key}`] = false; // manual click always wins over Smart Logic
...
window.toggleSmm = function (btn) {
    ...
    btn.classList.remove('auto-picked');
    clearSmmReasonTag(btn);
    peData.smm[key] = btn.classList.contains('sel');
    peAutoFlags.smm[key] = false; // manual click always wins over Smart Logic
...
window.setMarketState = function (btn) {
    document.querySelectorAll('.mstate-btn').forEach(b => {
        b.classList.remove('sel-bull','sel-bear','sel-neut','auto-picked');
    });
    btn.classList.add(btn.dataset.cls);
    peData.mstate = btn.dataset.val;
    peAutoFlags.mstate = false; // manual click always wins over Smart Logic
...
window.setVolatility = function (btn) {
    ...
    peAutoFlags.vol = false; // manual click always wins over Smart Logic
