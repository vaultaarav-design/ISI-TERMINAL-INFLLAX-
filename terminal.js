...
// ──────────────────────────────────────────────
// MAE / MFE — Trade Excursion Scale (manual, trader-observed, real per-trade data)
// ──────────────────────────────────────────────
window.updateMaeMfeDisplay = function () {
    const slider = document.getElementById('maeMfeSlider');
    const valEl  = document.getElementById('maeMfeValue');
    const hintEl = document.getElementById('maeMfeHint');
    const outcome = document.getElementById('outcome')?.value;
    const v = parseInt(slider.value, 10);

    valEl.textContent = (v > 0 ? '+' : '') + v + '%';
    valEl.style.color = v < 0 ? '#ff5252' : (v > 0 ? '#00c805' : 'var(--gold)');

    if (outcome === 'Target') {
        hintEl.textContent = 'Trade WON — kitna % pehle Stop-Loss ki taraf gaya tha before target hit hua? (left/red side pe mark karo)';
    } else if (outcome === 'Stop Loss') {
        hintEl.textContent = 'Trade LOST — kitna % pehle Target ki taraf gaya tha before stop hit hua? (right/green side pe mark karo)';
    } else {
        hintEl.textContent = 'Break Even — dono taraf thoda movement ho sakta hai, jo bhi zyada relevant laga wahi mark karo.';
    }
};
document.addEventListener('DOMContentLoaded', function () {
    const outcomeSel = document.getElementById('outcome');
    if (outcomeSel) outcomeSel.addEventListener('change', window.updateMaeMfeDisplay);
});
...
window.handleSaveAction = async function () {
    ...
    try {
        ...
        const trade = {
            ...
            sl:              pe0?.stopLoss   ?? null,
            tp:              pe0?.targetZone ? parseFloat(pe0.targetZone) : null,
            riskAmt:         pe0?.riskAmt    ?? null,
            rrPlanned:       pe0?.rrPlanned  || null,
            mstate:          pe0?.mstate     || '',
            volatility:      pe0?.volatility || '',
            sessionWindow:   pe0?.sessionWindow   || null,
            sessionViolation:pe0?.sessionViolation|| false,
            preEntryTime:    pe0?.savedAt    || null,
            maeMfe:          parseInt(document.getElementById('maeMfeSlider')?.value || '0', 10),
        ...
        await writeStats(selectedClusterId, selectedNodeIdx, {
            currentBal: newBal, trades: newT, wins: newW, winRate: newWR, net: newNet
        });
...
// PDF EXPORT
// ──────────────────────────────────────────────
// Helper: fetch Storage URL → base64 for PDF embed
...
window.downloadSinglePDF = async function (t) {
    ...
    if (t.image) {
        try {
            ...
            if (imgData) doc.addImage(imgData, imgData.includes('image/png')?'PNG':'JPEG', 15, 20, 180, 130);
        } catch(e) {}
    }
...
// ──────────────────────────────────────────────
// NOTE: Oath + Password gate has MOVED to Daybook (index.html / daybook.js).
// Terminal is now a protected page — the auth guard in terminal.html's
// <head> already bounces unauthenticated visitors back to index.html
// before this script even runs. getAllPermKeys() above is kept because
// other terminal features (temp-password issuing, key management) still
// reference it.
// ──────────────────────────────────────────────
